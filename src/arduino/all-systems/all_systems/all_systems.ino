...
#define R_WINGSTOP_PIN 35
#define L_WINGSTOP_PIN 34
...
void setup()
{
  ...
  leftwingstop.attach(L_WINGSTOP_PIN);
  rightwingstop.attach(R_WINGSTOP_PIN);

  pinMode(L_FOREARM_POT_PIN, INPUT);
  pinMode(L_ELEV_POT_PIN, INPUT);
  pinMode(R_FOREARM_POT_PIN, INPUT);
  pinMode(R_ELEV_POT_PIN, INPUT);
  pinMode(L_WRISTBRO_PIN, INPUT);
  pinMode(R_WRISTBRO_PIN, INPUT);
  pinMode(L_WINGSTOP_PIN, INPUT);
  pinMode(R_WINGSTOP_PIN, INPUT);
...
void loop() // run over and over
{
  ...
  leftwingstop.write(leftWingAngle);
  rightwingstop.write(rightWingAngle);
