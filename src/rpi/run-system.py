...
# Controller/arm state variables
l3_vert_old = 0
l3_horz_old = 0
r3_vert_old = 0
r3_horz_old = 0
...
# Inverse Kinematics Demo Function
def ik_update():
    global r3_vert_pre, r3_horz_pre, r2_pre
    # Simulate time-based hand position
    overall_time = time.time() - start_time
    local_time = overall_time % 10
    t = local_time
    x = 20 + 5 * np.sin(2 * np.pi * t / 10)
    y = -20

    # Arm geometry
    r = 10
    x0 = r * np.cos(np.deg2rad(80))
    y0 = r * np.sin(np.deg2rad(80))
    L2 = 2

    L3 = np.sqrt(x**2 + y**2)
    theta3 = np.rad2deg(np.arctan2(y, x))
    theta2 = 200 - theta3
    L1 = np.sqrt(r**2 - 2 * L2 * (x0 * np.cos(np.deg2rad(theta2)) + y0 * np.sin(np.deg2rad(theta2))) + L2**2)

    forearm = L1 - 3
    upperArm = L3 - 3

    # Map to controller input range
    r3_horz_pre = (65536 / 18) * forearm - 32768
    r3_vert_pre = (65536 / 18) * upperArm - 32768
    r2_pre = (65536 / 18) * L3 - 32768
...
class MyController(Controller):
    ...
    # PS button toggles kinematic mode
    def on_playstation_button_press(self):
        global kinematic_mode
        kinematic_mode = not kinematic_mode
        print(f"Kinematic mode {'enabled' if kinematic_mode else 'disabled'}")

# Start threads
threading.Thread(target=filterThread, daemon=True).start()
threading.Thread(target=ikThread, daemon=True).start()
